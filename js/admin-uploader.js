/* js/admin-uploader.js
   ...
   - Chunked upload stub: splits file into chunks and POSTs to /api/uploads/chunk
     (server endpoints must be implemented separately; this is a resumable-ready scaffold).
   - Accessible UI updates (aria-live, focus management), keyboard support.
   - Designed to be robust and visually polished when paired with CSS (admin-dashboard.css).
*/
(function(){
  'use strict';
  ...
  document.addEventListener('DOMContentLoaded', ()=>{
    ...
    function showFile(file){ currentFile = file; preview.classList.remove('hidden'); preview.setAttribute('aria-hidden','false'); previewName.textContent = file.name; previewSize.textContent = humanSize(file.size); log('File ready: '+file.name);
      ...
      if(type.startsWith('video/')){
        ...
        vid.addEventListener('loadeddata', ()=>{ try{ const canvas = document.createElement('canvas'); canvas.width = 320; canvas.height = 180; const ctx = canvas.getContext('2d'); ctx.drawImage(vid, Math.min(1, vid.currentTime||0), 0, canvas.width, canvas.height); const img = new Image(); img.src = canvas.toDataURL('image/jpeg',0.7); img.alt = file.name; img.width = 240; previewMedia.innerHTML=''; previewMedia.appendChild(img); URL.revokeObjectURL(vid.src); }catch(e){ console.warn('Thumbnail generation failed', e); } }, { once:true });
    ...
    async function startUpload(file){ isUploading = true; log('Starting upload: '+file.name); progressBar.style.width='0%'; progress.setAttribute('aria-valuenow','0');
      ...
      try{
        while(offset < total){ const chunk = file.slice(offset, offset + CHUNK_SIZE); const form = new FormData(); form.append('uploadId', uploadId); form.append('part', String(part)); form.append('chunk', chunk);
          // example fetch endpoint - server should accept multipart chunk uploads
          await fetch('/api/uploads/chunk', { method:'POST', body:form, signal: uploadController.signal }).then(r=>{ if(!r.ok) throw new Error('Upload chunk failed: '+r.status); });
        ...
        // finalize
        await fetch('/api/uploads/complete', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ uploadId, fileName: file.name }) });
